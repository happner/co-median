describe('functional tests', function () {
  ...
  it('tests the wildcard matching wildcard on both sides, issues', function(done){
    ...
    expect(comedian.matches('*/t', '/t*')).to.be(true);

    expect(comedian.matches('*test/', 'test/*')).to.be(true);

    expect(comedian.matches('Bs', 'B*Hs')).to.be(false);

    expect(comedian.matches('*Bs', 'B*Hs')).to.be(true);

    expect(comedian.matches('*/wildcard*complex/*', '*/*/co*mplex*')).to.be(true);

    expect(comedian.matches('*e*', '*y*')).to.be(false);

    expect(comedian.matches('test*', '*test')).to.be(true);

    expect(comedian.matches('*test/mat', '*p*st*')).to.be(false); //leave top

    expect(comedian.matches('*test/match', '/test/mat*')).to.be(true); // leave left

    expect(comedian.matches('*t', '*d')).to.be(false);

    expect(comedian.matches('*atch', '*lah')).to.be(false);
    ...
    expect(comedian.matches('i*g1', 'str*ing*')).to.be(true); //leave right
  ...
  it('tests the wildcard matching wildcard on both sides, negatives', function(done){
    ...
    expect(comedian.matches('*test/mat', '*p*st*')).to.be(false);
  ...
  it('tests wildcard matching, edge failure cases', function (done) {
    ...
    //
    expect(comedian.matches("0y", "*y")).to.be(true);
    //
